 ...
 *
 *   pnpm exec tsx scripts/verify-sync.ts
 *
...
async function main() {
  ...
  assert(pulled!.accounts.some((a) => a.id === 'acc_verify_1'), 'pushed account is visible via pull');
  assert(pulled!.categories.some((c) => c.id === 'cat_verify_1'), 'pushed category is visible via pull');
  ...
  assert(restored.accounts.every((a) => a.syncedAt !== null), 'every restored account is marked synced');
  assert(restored.accounts.find((a) => a.id === 'acc_verify_1') !== undefined, 'restored AppData contains the test account');
  ...
  assert(reconciled.accounts.find((a) => a.id === 'acc_verify_1')?.syncedAt !== null, 'pushed account marked synced locally');
  ...
  assert(!finalPull!.accounts.some((a) => a.id === 'acc_verify_1'), 'test account is gone from the server');
  assert(finalPull!.disciplineState === null, 'test disciplineState is gone from the server');
  assert(!finalPull!.categories.some((c) => c.id === 'cat_verify_1'), 'test category is gone from the server');
