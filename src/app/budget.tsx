...
import { AmountText, FabSafeScrollView, Overline, PressFeedback, ScreenHeader } from '@/components/ui';
...
export default function BudgetScreen() {
  ...
  return (
    <View style={styles.root}>
      <ScreenHeader title="Budget" paddingTop={isWeb ? 67 : insets.top + 10} onBack={() => router.back()} />

      <View style={styles.monthSwitcher}>
        <Pressable onPress={() => setMonthDate((d) => addMonths(d, -1))} style={styles.monthArrow}>
      ...
      <FabSafeScrollView contentContainerStyle={{ paddingHorizontal: layout.gutter, paddingTop: layout.sectionGap }}>
        <View style={styles.summaryRow}>
          <View style={styles.summaryTile}>
            ...
            <AmountText amount={summary.totalIncome} color={palette.positive} style={styles.summaryAmount} />
          </View>
          <View style={styles.summaryTile}>
            <Text style={type.caption}>Expenses</Text>
            <AmountText amount={summary.totalExpenses} kind="expense" style={styles.summaryAmount} />
          </View>
          <View style={styles.summaryTile}>
            <Text style={type.caption}>Net</Text>
            <AmountText amount={summary.netSavings} style={styles.summaryAmount} />
          </View>
        </View>
            ...
            <View key={row.categoryId} style={styles.categoryCard}>
              <View style={styles.categoryRow}>
                ...
                <Text style={styles.categoryActual}>
                  ...
                  />
                  <Text style={type.caption}> / {CURRENCY_SYMBOL}{row.planned.toFixed(2)}</Text>
                </Text>
              </View>
              ...
              {row.planned > 0 && (
                <View style={styles.progressTrack}>
                  <View
                    style={[
                      styles.progressFill,
                      { width: `${progress * 100}%`, backgroundColor: overBudget ? palette.alert : palette.link },
                    ]}
                  />
                </View>
              )}
              ...
              {isEditing ? (
                <View style={styles.editRow}>
                  ...
                  <Pressable onPress={saveEdit} style={styles.editAction} hitSlop={6}>
                    <Feather name="check" size={18} color={palette.link} />
                  </Pressable>
                  <Pressable onPress={() => setEditingCategoryId(null)} style={styles.editAction} hitSlop={6}>
                ...
                >
                  <Feather name="edit-2" size={12} color={palette.link} style={styles.setBudgetIcon} />
